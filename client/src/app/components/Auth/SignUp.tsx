...
const SignUp: FC<Props> = ({ setRoute }) => {
  ...
  // Use effect for handling successful registration or error messages
  useEffect(() => {
    if (isSuccess) {
      const message = data?.message || "Registration successful";
      toast.success(message);
      setRoute("Verification"); // Navigate to verification on success
    } 
   if (error) {
      if("data" in error){
    ...
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3 relative">
          ...
          />
          {show ? (
            <AiOutlineEye className="absolute bottom-5 right-2 z-1 cursor-pointer" size={20} onClick={() => setShow(false)} />
          ) : (
            <AiOutlineEyeInvisible className="absolute bottom-5 right-2 z-1 cursor-pointer" size={20} onClick={() => setShow(true)} />
          )}
        </div>

        {formik.errors.password && formik.touched.password && (
          <span className="text-red-500 pt-2 block">{formik.errors.password}</span>
        )}
